...
* reads a local `osinfo.json` config
...
Copy one of the existing examples in the `examples/` dir and modify it to fit your needs. The file must live in the root of the repo and be named `osinfo.json`.
``` Bash
cp examples/win11.json osinfo.json
```
...
The Docker setup assumes you will provide your own `osinfo.json` and mount persistent `downloads/` and `outputs/` directories into the container.
...
docker run --rm -it \
  ...
  -v "$PWD/osinfo.json:/git/wudd/osinfo.json:ro" \
