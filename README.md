...
At a high level, wudd:
* reads a local `osinfo.toml` config
* searches Microsoft Update Catalog for matching updates
* writes the results to `outputs/`
* optionally downloads the update files to `downloads/`
...
wudd works by parsing a TOML file that contains OS version, release, architecture, update type and date range of updates that the user wants. I have created several examples in the `examples/` dir, but you will most likely want to modify and create your own based on your needs.
...
Copy one of the existing examples in the `examples/` dir and modify it to fit your needs. The file must live in the root of the repo and be named `osinfo.toml`.
``` Bash
cp examples/win11.toml osinfo.toml
```
...
The `osinfo.toml` configuration for those requirements would look like the following
``` toml
[[targets]]
os = "11"
archs = ["x64"]
...
[targets.releases."24H2"]
start = "2024-12"
end = "latest_patch_tuesday"
```

Using `end = "latest_patch_tuesday"` inside each release entry keeps the config current without needing a monthly edit.
Using `releases` and `archs` lets one block fan out to multiple release and architecture combinations.
`releases` is now a set of per-release tables, which keeps the dates aligned automatically and stays easy to scan.
...
Before running wudd locally, make sure you have:
* Python 3
* `requests` and `selenium`
* A supported browser (`chrome` or `firefox`)
* A Selenium-compatible browser driver on your system, if your local setup does not provide one automatically
...
Run the script without downloading anything to just generate the output files:
``` Bash
python3 wudd.py
```

Download and validate the update payloads too:
``` Bash
...
usage: wudd.py [-h] [--browser {chrome,firefox}] [--clean] [--download] [--foreground] [--latest] [--logging {debug,info,warning,error,critical}]
               [--no-snapshot-cache] [--skipsha1] [--workers WORKERS]
...
## Repo Directory Structure
```
bin/ - Small launcher scripts
docker/ - Files for building and running wudd in Docker
downloads/ - Directory updates are downloaded to
examples/ - Example toml files to help get you started configuring what updates you want to download
images/ - Images for the repo
outputs/ - Structured output generated by the current run and kept in sync over time
stored/ - Precomputed snapshot of many updates committed to the repo for offline use
wuddlib/ - Library modules for catalog scraping, downloads, outputs, and orchestration
wudd.py - CLI entrypoint
```

`outputs/` and `stored/` serve different jobs. `outputs/` is the live working set that gets refreshed by your local runs, while `stored/` is a repo-maintained snapshot you can use when you do not want to run the scraper at all.
...
<!-- DOCKER -->
## Docker
The files under `docker/` are a starting point for containerized runs.

The Docker setup assumes you will provide your own `osinfo.toml` and mount persistent `downloads/` and `outputs/` directories into the container.

Typical usage looks like this:
``` Bash
docker build -f docker/Dockerfile -t tuxthepenguin84/wudd:latest .
docker run --rm -it \
  -v "$PWD/downloads:/git/wudd/downloads" \
  -v "$PWD/outputs:/git/wudd/outputs" \
  -v "$PWD/osinfo.toml:/git/wudd/osinfo.toml:ro" \
  tuxthepenguin84/wudd:latest --download
```

If you use Docker, make sure the image has a Selenium-capable browser available. The repository's Docker files are intentionally lightweight and may need adjustment for your environment.
      ...
      "x64": {
        "2025-01": {
          ...
          "aaa20f4c-2672-44ef-99b7-4d40d5685101": {
            "title": "2025-06 Cumulative Update for Windows 10 Version 22H2 for x64-based Systems (KB5063159)",
            "kb": "KB5063159",
            "files": [
              "https://catalog.s.download.windowsupdate.com/c/msdownload/update/software/updt/2025/06/windows10.0-kb5063159-x64_50ea3cbc0ed169387390d7cc57dc8e53cae2d183.msu"
            ],
            "sha1": [
              "50ea3cbc0ed169387390d7cc57dc8e53cae2d183"
            ]
          },
